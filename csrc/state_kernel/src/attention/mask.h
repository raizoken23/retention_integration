...
template <bool LogSpace=true, typename Engine, typename Layout>
            ...
            if (col_idx >= max_seqlen_k) {
                ...
                for (int mi = 0; mi < size<0>(tensor); ++mi) {
                    if constexpr (LogSpace) {
                        tensor(make_coord(mi, j), make_coord(j, nj)) = -INFINITY;
                    } else {
                        tensor(mi, make_coord(j, nj)) = 0.0f;
                    }
                }
            }
        }
...
template <int warp_row_stride, bool LogSpace=true, typename Engine, typename Layout>
            ...
            for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                ...
                for (int j = 0; j < size<1, 0>(tensor); ++j) {
                    ...
                    if (col_idx >= col_idx_limit_right) {
                        tensor(make_coord(i, mi), make_coord(j, nj)) = LogSpace ? -INFINITY : 0.0f;
                    }
                }
            }
        }
...
struct Mask {
    ...
    template <bool Causal_mask=false, bool Is_even_MN=true, bool NormalSpace=false, typename Engine, typename Layout>
    __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout> &tensor_,
        ...
        if constexpr (Need_masking) {
            ...
            Tensor tensor = make_tensor(tensor_.data(), power::convert_layout_acc_rowcol(tensor_.layout()));
                    ...
                    for (int j = 0; j < size<1, 0>(tensor); ++j) {
                        ...
                        for (int mi = 0; mi < size<0>(tensor); ++mi) {
                            ...
                            if constexpr (!Is_even_MN) {
                                if (col_idx >= max_seqlen_k) { tensor(mi, make_coord(j, nj)) = NormalSpace ? 0.0f : -INFINITY; }
                            }
                        }
                    }
                }
                        ...
                        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                            ...
                            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                                ...
                                if constexpr (Causal_mask) {
                                    if (col_idx >= col_idx_limit_right) {
                                        tensor(make_coord(i, mi), make_coord(j, nj)) = NormalSpace ? 0.0f : -INFINITY;
                                    }
                                }
